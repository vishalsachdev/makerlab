...
### 2026-05-22
- Completed: Removed the summer camp instructor hiring call (positions filled) — deleted the "We're Hiring Camp Instructors!" banner from `summer.html`, archived `jobs.html` + `camp-instructor-3d-printing.html` + `camp-instructor-robotics.html` to `archive/pages/`, dropped the broken job-posting links from `camp-instructor-schedule.html` (kept as operational staff schedule), and cleaned `api/pages.json` (removed stale `/summer-jobs.html` active entry, added archived entry). `validate_agent_data.py` passes. Commit 62b5ab6, pushed. Also documented the refund-authorization memo workflow (Kip Mecum) in CLAUDE.md.
- Next: **Finish the Summers cancellation** (carried over, uncommitted) — FormBuilder answer edit to remove `MINECRAFT_JUN2` + `ROBOTARM_JUN1` for Landon Summers, run `update_availability.py`, send the `data/refund-memo-summers.md` memo to Kip via Outlook, then commit `cancellations.csv` + memo. Still pending: waitlist tracking approach, ChambanaMoms campaign images, monthly Podio blog workflow. When camps start (Jun 1), re-enable Podio order form on `online-ordering.html` + update order pages with summer hours.
