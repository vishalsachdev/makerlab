...
### 2026-05-22
- Completed: Removed the summer camp instructor hiring call (positions filled) — deleted the "We're Hiring Camp Instructors!" banner from `summer.html`, archived `jobs.html` + `camp-instructor-3d-printing.html` + `camp-instructor-robotics.html` to `archive/pages/`, dropped the broken job-posting links from `camp-instructor-schedule.html` (kept as operational staff schedule), and cleaned `api/pages.json` (removed stale `/summer-jobs.html` active entry, added archived entry). `validate_agent_data.py` passes. Commit 62b5ab6, pushed. Documented the refund-authorization memo workflow (Kip Mecum) in CLAUDE.md. **Summers cancellation:** confirmed 5/12 request via Podio (within 21-day full-refund window, cutoff 5/18), sent reply to Nichole via GlobiMail, drafted + sent Kip Mecum refund memo ($410 = $205×2, IPay txn REDACTED), logged both rows in `data/cancellations.csv`. Verified no FormBuilder camps start before Jun 1. Commit aebd036, pushed.
- Next: **Finish Summers cancellation** — FormBuilder answer edit to remove `MINECRAFT_JUN2` + `ROBOTARM_JUN1` from Landon's response `REDACTED` (do NOT use the answer-editor — it collapses the multi-week question to single-select and destroys the other 4 Minecraft weeks; edit manually). Then run `python3 scripts/update_availability.py` to refresh badges and send a follow-up to Nichole confirming the seats are freed. Optional cleanup: duplicate Feb 27 Summers PaymentProcessing response, stray "test session for payment" option in Adventures camp. Still pending: waitlist tracking approach, ChambanaMoms campaign images, monthly Podio blog workflow. When camps start (Jun 1), re-enable Podio order form on `online-ordering.html` + update order pages with summer hours.
