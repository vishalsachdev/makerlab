...
### 2026-07-06
- Completed: **Jul 6–10 welcome emails (with a roster mistake caught & corrected) + plans cleanup.** (All family identities/emails in gitignored `data/parent-emails/jul-6-10-*.md`, not here.) Two camps started Jul 6: Adventures (AM, 8 campers) and Reachy (PM, 6 campers); 2 all-day campers. 🔴 **First batch used the wrong Reachy roster**: assumed `AIROBOTICS_JUL1` = Jul 6–10, but Reachy's July codes are NON-chronological (`JUL1`=Jul 27–31, `JUL2`=Jul 6–10 — documented in `scripts/update_availability.py` CODE_TO_SESSION, which wasn't consulted; `data/registrations-july-2026.csv` dated labels would also have caught it). 3 wrong/partial emails went out before the catch. Corrected set = 4 compose windows: Reachy welcome (4 families), combined all-day (2 families, supersede note), correction to the 4 wrongly-emailed Jul 27–31 families, correction to the one AM-only family that got the full-day email. Final rosters verified: snapshot-via-CODE_TO_SESSION vs dated CSV = zero mismatches. Also: archived blender-ai-coach plan, deleted completed cloudflare-migration plan, kept podio-migration. Robot Arm phantom-seat parent successfully booked.
- Next: (0) **⚠️ EXECUTE THE PENDING JUNE TRANSFER — accepted but never processed** (identities in gitignored `data/refund-memo-hodges.md` + `data/parent-emails/owen-hodges-transfer.md`): parent accepted moving camper from Robot Arm Jun 22–26 → GenAI Jul 20–24 (even swap, no refund), but FormBuilder still shows the old session (response ends `…010a0cab`). GenAI Jul 20–24 has 2 seats left. Do: DETAILS-view checkboxes (response active, not cancelled) — uncheck ROBOTARM_JUN2, check GENAI3D_JUL1; reply to parent; delete the contingent refund memo; log transfer in cancellations.csv; family then joins the GenAI welcome email (send Wed Jul 15). The acceptance sat in the uimakerlab mailbox (not indexed by mailcorpus). (1) **Confirm all 4 final Jul 6 emails actually went out** — user was diffing sent items at session end; unresolved. (2) **One Adventures family (registered after the 5/30 dump) never got a welcome email** — address not in local data; pull from FormBuilder admin or uimakerlab OWA. (3) **Build camp-welcome-email skill**: run Wednesday of the week before each camp; next send **Wed Jul 8** for Jul 13–17 (Minecraft PM + Robot Arm AM). Must import CODE_TO_SESSION from `scripts/update_availability.py` (never assume codes chronological), refresh snapshot first, cross-assert rosters against dated session labels, detect all-day campers, flag parents missing from the email source, reuse `data/parent-emails/` templates + compose-outlook-email pattern. (4) Phantom cleanup once Robot Arm seat state settles (Max Reg 7→6). Carried: Outlook re-auth; GenAI silent-fallback form fix; re-enable Podio order form; mailcorpus Phase 3.
