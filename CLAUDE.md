...
- Next: (1) **Thera-Solutions job:** Bayu/Sahib to quote the mini redesign and confirm the 25-unit build; reply to Celine. (2) **Thursday Sep 3 door notice** (4 PM open) is a physical task. (3) Unverified since Aug 4, check before assuming done: exec summary to Peecher (due Aug 7), parent feedback nudge/one-pager, recap post publish, watch-cleaner post (Podio order 2210). (4) Roadmap: Podio migration (drop GlobiMail first), monthly blog workflow from Podio orders. (5) Site-review decisions from the 08-03 archive entry.
